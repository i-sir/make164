...
class RoadPriceInit extends Base
{
    ...
    */
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function edit_post($params,$where=[])
        {
            ...
            if (!empty($params["id"])) {
                ...
                $result                = $RoadPriceModel->strict(false)->update($params);
                if($result) $result = $item["id"];
            }elseif(!empty($where)){
                //传入where条件,根据条件更新数据
                $params["update_time"] = time();
                $result                = $RoadPriceModel->where($where)->strict(false)->update($params);
                if ($result) $result = $item["id"];
            }else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $RoadPriceModel->strict(false)->insert($params,true);
            }
        ...
        */
        ...
        if ($type == 1) $result = $RoadPriceModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $RoadPriceModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $RoadPriceModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $RoadPriceModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
